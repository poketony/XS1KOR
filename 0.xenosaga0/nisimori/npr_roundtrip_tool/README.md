...
Drag a `.npr` file onto `NPR_Drop_Here.bat` to extract it.
...
- No-edit NPR rebuild copies `original.npr` byte-for-byte.

Files in an extract folder:

- `npr_meta.json`: rebuild metadata
- `original.npr`: untouched compressed input copy
- `original_payload.bin`: decompressed ARX payload used as rebuild base
- `entries\NN_name\original.bin`: raw top-level entry bytes
...
```powershell
python C:\Users\JO\XS1KOR\codex-lab\npr_roundtrip_tool\npr_roundtrip_tool.py C:\path\to\file.npr
python C:\Users\JO\XS1KOR\codex-lab\npr_roundtrip_tool\npr_roundtrip_tool.py C:\path\to\file_npr_extract
```
